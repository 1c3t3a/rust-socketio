...
DEBUG=* node engine-io-no-ws.js &
status=$?
if [ $status -ne 0 ]; then
  echo "Failed to start engine.io polling: $status"
  exit $status
fi
echo "Successfully started engine.io polling instance"
...
while sleep 60; do
  ...
  PROCESS_1_STATUS=$?
  ps aux |grep engine-io-no-ws.js |grep -q -v grep
  PROCESS_2_STATUS=$?
  ps aux |grep engine-io.js |grep -q -v grep
  PROCESS_3_STATUS=$?
  
  ps aux |grep engine-io-no-ws.js
